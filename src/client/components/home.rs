...
impl Component for Home {
    ...
    fn draw(&mut self, frame: &mut Frame, area: Rect) -> Result<()> {
        ...
        // Calculate centering margins - more centering as width increases
        let center_margin = if available_width <= 80 {
            (available_width * 2) / 100 // 2% centering for narrow terminals
        } else if available_width <= 120 {
            (available_width * 8) / 100 // 8% centering for medium terminals
        } else if available_width <= 160 {
            (available_width * 15) / 100 // 15% centering for wide terminals
        } else {
            // For very wide terminals, increase centering significantly
            (available_width * 20) / 100 // 20% centering for ultra-wide terminals
        };

        // Calculate message area width after centering
        let message_area_width = available_width.saturating_sub(center_margin * 2);
                ...
                match style {
                    MessageStyle::Sent => {
                        ...
                        for (_line_index, wrapped_line) in wrapped_lines.iter().enumerate() {
                            ...
                            let padding = if content_len < message_area_width {
                                center_margin + message_area_width.saturating_sub(content_len)
                            } else {
                                0
                            };
                    ...
                    MessageStyle::Received => {
                        ...
                        for wrapped_line in wrapped_lines.iter() {
                            let bubble_content = format!("  {}  ", wrapped_line);
                            // Create line with left centering margin and colored content
                            let line = Line::from(vec![
                                Span::raw(" ".repeat(center_margin)), // Left centering margin
                                Span::styled(bubble_content, style),  // Colored content only
                            ]);
                            lines.push(line);
                        }
                    ...
                    MessageStyle::DMReceived => {
                        ...
                        for wrapped_line in wrapped_lines {
                            ...
                            lines.push(Line::from(vec![
                                Span::raw(" ".repeat(center_margin)), // Left centering margin
                                Span::styled(bubble_content, style),  // Colored content
                            ]));
                        }
                    }
                };
