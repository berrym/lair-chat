...
use crate::transport::{
    add_text_message, ConnectionConfig, ConnectionObserver, ConnectionStatus, TransportError,
    TuiObserver, CLIENT_STATUS, MESSAGES,
};
use tui_input::Input;
...
impl ConnectionObserver for LegacyTransportObserver {
    fn on_message(&self, message: String) {
        tracing::info!(
            "DEBUG: LegacyTransportObserver received message: {}",
            message
        );
    ...
    fn on_status_change(&self, connected: bool) {
        tracing::info!(
            "DEBUG: LegacyTransportObserver connection status changed: {}",
            connected
        );
    }
}
...
#[deprecated(
    since = "0.5.1",
    note = "Use ConnectionManager.connect() directly instead. This compatibility function will be removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
pub async fn connect_client_compat(
    input: Input,
    ...
    add_text_message(format!(
        "Connecting to {} using legacy transport...",
        address
    ));
    ...
    match timeout(Duration::from_secs(5), TcpStream::connect(address)).await {
        Ok(Ok(_stream)) => {
            ...
            let status = crate::transport::CLIENT_STATUS
                .lock()
                .unwrap()
                .status
                .clone();
            if status == crate::transport::ConnectionStatus::CONNECTED {
                ...
                if let Some(manager) = COMPAT_CONNECTION_MANAGER.lock().await.as_mut() {
                    ...
                    tokio::task::block_in_place(|| {
                        tokio::runtime::Handle::current().block_on(async {
                            ...
                            if status != crate::transport::ConnectionStatus::CONNECTED {
                                tracing::info!(
                                    "DEBUG: Updating ConnectionManager status to CONNECTED"
                                );
                ...
                Err(TransportError::ConnectionError(std::io::Error::new(
                    ...
                    format!("Transport failed to connect to {}", address),
                )))
            }
        }
        Ok(Err(e)) => {
...
#[deprecated(
    since = "0.5.1",
    note = "Use AuthManager.login() with ConnectionManager instead. This compatibility function will be removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
...
#[deprecated(
    since = "0.5.1",
    note = "Use ConnectionManager directly instead. This compatibility function will be removed in v0.6.0."
)]
...
pub async fn sync_connection_status() {
    ...
    if let Some(manager) = COMPAT_CONNECTION_MANAGER.lock().await.as_mut() {
        ...
        if current_status != legacy_status {
            tracing::info!(
                "DEBUG: Syncing ConnectionManager status from {:?} to {:?}",
                current_status,
                legacy_status
            );
            ...
            if legacy_status == crate::transport::ConnectionStatus::CONNECTED {
                tracing::info!(
                    "DEBUG: Legacy transport is CONNECTED but ConnectionManager is {:?}",
                    current_status
                );
            } else {
                tracing::info!(
                    "DEBUG: Legacy transport is DISCONNECTED but ConnectionManager is {:?}",
                    current_status
                );
            }
        }
...
#[deprecated(
    since = "0.5.1",
    note = "Use ConnectionManager.disconnect() directly instead. This compatibility function will be removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
...
#[deprecated(
    since = "0.5.1",
    note = "Use ConnectionManager.get_status() directly instead. This compatibility function will be removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
...
#[deprecated(
    since = "0.5.1",
    note = "Use ConnectionManager.send_message() directly instead. This compatibility function will be removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
pub async fn send_message_compat(message: String) -> Result<(), TransportError> {
    use crate::transport::{add_outgoing_message, ConnectionStatus, CLIENT_STATUS};

    // Skip ConnectionManager registration to prevent transport interference
...
#[deprecated(
    since = "0.5.1",
    note = "This function will be removed when compatibility layer is eliminated in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
...
#[deprecated(
    since = "0.5.1",
    note = "This cleanup function will no longer be needed when compatibility layer is removed in v0.6.0. See LEGACY_CODE_AUDIT_AND_DEPRECATION_PLAN.md for migration guidance."
)]
    ...
    async fn test_disconnect_compat_not_connected() {
        ...
        if let Ok(messages) = MESSAGES.try_lock() {
            assert!(messages
                .text
                .iter()
                .any(|msg| msg.contains("Not connected")));
        }
    }
}
