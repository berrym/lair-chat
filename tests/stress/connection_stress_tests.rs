...
impl StressTestMetrics {
    ...
    fn report(&self) {
        ...
        println!("Errors Encountered: {}", self.errors_encountered.load(Ordering::Relaxed));
    }
}
...
async fn test_massive_concurrent_connections() {
    ...
    let success_rate = metrics.successful_connections.load(Ordering::Relaxed) as f64 / MAX_CONNECTIONS as f64;
    assert!(success_rate >= 0.95, "Connection success rate too low: {:.2}%", success_rate * 100.0);
}
...
async fn test_connection_stability_under_load() {
    ...
    assert!(message_success_rate >= 0.95, "Message success rate too low: {:.2}%", message_success_rate * 100.0);
}
...
async fn test_rapid_connect_disconnect() {
    ...
    let success_rate = metrics.successful_connections.load(Ordering::Relaxed) as f64 / total_attempts as f64;
    assert!(success_rate >= 0.95, "Connect/disconnect success rate too low: {:.2}%", success_rate * 100.0);
}
...
async fn test_connection_recovery_under_stress() {
    ...
    let success_rate = metrics.successful_connections.load(Ordering::Relaxed) as f64 / total_attempts as f64;
    assert!(success_rate >= 0.90, "Recovery success rate too low: {:.2}%", success_rate * 100.0);
}